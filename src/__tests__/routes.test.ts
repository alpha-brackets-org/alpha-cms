...
// Helpers & Data
import { makeReq, clearCollections, seedDefaultPortfolio, seedUser } from './test-helpers';
import {
  mockAdminUser,
  mockBlogData,
  mockCaseStudyData,
...
import { GET as getBlog, PATCH as patchBlog, DELETE as deleteBlog } from '@/app/api/blogs/[id]/route';
import { GET as getCaseStudies, POST as postCaseStudy } from '@/app/api/case-studies/route';
import { GET as getCaseStudy, PATCH as patchCaseStudy, DELETE as deleteCaseStudy } from '@/app/api/case-studies/[id]/route';
...
import { GET as getFaq, PATCH as patchFaq, DELETE as deleteFaq } from '@/app/api/faqs/[id]/route';
import { GET as getTestimonials, POST as postTestimonial } from '@/app/api/testimonials/route';
import { GET as getTestimonial, PATCH as patchTestimonial, DELETE as deleteTestimonial } from '@/app/api/testimonials/[id]/route';
import { GET as getCategories, POST as postCategory } from '@/app/api/categories/route';
import { PATCH as patchCategory, DELETE as deleteCategory } from '@/app/api/categories/[id]/route';
import { GET as getPortfolios, POST as postPortfolio } from '@/app/api/portfolios/route';
import { GET as getPortfolio, PATCH as patchPortfolio, DELETE as deletePortfolio } from '@/app/api/portfolios/[id]/route';
...
import { GET as getLead, PATCH as patchLead, DELETE as deleteLead } from '@/app/api/leads/[id]/route';
import { GET as exportLeads } from '@/app/api/leads/export/route';
import { GET as getSubscribers } from '@/app/api/subscribers/route';
import { PATCH as patchSubscriber, DELETE as deleteSubscriber } from '@/app/api/subscribers/[id]/route';
...
import { GET as getCampaign, DELETE as deleteCampaign } from '@/app/api/campaigns/[id]/route';
import { POST as sendCampaign } from '@/app/api/campaigns/[id]/send/route';
import { GET as getMedia, POST as postMedia, DELETE as bulkDeleteMedia } from '@/app/api/media/route';
import { PATCH as patchMedia, DELETE as deleteMedia } from '@/app/api/media/[id]/route';
...
describe('CMS Endpoint Integration Tests', () => {
  ...
  beforeEach(async () => {
    await clearCollections(mongoose.connection.db);
    testPortfolioIdStr = await seedDefaultPortfolio(mongoose.connection.db);
  ...
  describe('Users Endpoint Group', () => {
    it('GET /api/users requires admin role', async () => {
      // Unauthenticated
      const unauthRes = await getUsers(makeReq('http://localhost/api/users'), { params: Promise.resolve({}) });
      expect(unauthRes.status).toBe(403);

      // Authenticated as Admin
      mockCookieToken = testAdminToken;
      const adminRes = await getUsers(makeReq('http://localhost/api/users'), { params: Promise.resolve({}) });
      expect(adminRes.status).toBe(200);
      const usersList = await adminRes.json();
    ...
    it('POST /api/users, PATCH /api/users/[id], and DELETE /api/users/[id] work correctly', async () => {
      ...
      const newUserIdStr = postData.id.toString();
  ...
  describe('Stats Endpoint Group', () => {
    it('GET /api/stats works', async () => {
      mockCookieToken = testAdminToken;
      const res = await getStats(makeReq('http://localhost/api/stats?months=6'), { params: Promise.resolve({}) });
      expect(res.status).toBe(200);
  ...
  describe('Blogs Endpoint Group', () => {
    it('GET /api/blogs and mutations work correctly', async () => {
      // GET
      const getRes = await getBlogs(makeReq('http://localhost/api/blogs'), { params: Promise.resolve({}) });
      ...
      const postRes = await postBlog(
        makeReq('http://localhost/api/blogs', 'POST', mockBlogData(testPortfolioIdStr)),
        { params: Promise.resolve({}) }
      );
      expect(postRes.status).toBe(201);
      const postData = await postRes.json();
      const blogId = postData.id || postData._id;

      // GET single
      const getSingleRes = await getBlog(makeReq(`http://localhost/api/blogs/${blogId}`), {
      ...
      const patchRes = await patchBlog(
        makeReq(`http://localhost/api/blogs/${blogId}`, 'PATCH', { title: 'Updated Title' }),
        { params: Promise.resolve({ id: blogId }) }
      );
      expect(patchRes.status).toBe(200);

      // DELETE
      const deleteRes = await deleteBlog(makeReq(`http://localhost/api/blogs/${blogId}`, 'DELETE'), {
        params: Promise.resolve({ id: blogId }),
      });
      expect(deleteRes.status).toBe(200);
    });
    ...
    it('handles pagination and filtering parameters correctly', async () => {
      ...
      // 1. Create a category
      const categoryResult = await mongoose.connection.db.collection(CollectionName.CATEGORIES).insertOne({
        name: 'Tech',
        slug: 'tech',
      ...
      await mongoose.connection.db.collection(CollectionName.BLOGS).insertMany([
        {
      ...
      const page1Res = await getBlogs(makeReq(`http://localhost/api/blogs?limit=2&page=1`), { params: Promise.resolve({}) });
      ...
      const page2Res = await getBlogs(makeReq(`http://localhost/api/blogs?limit=2&page=2`), { params: Promise.resolve({}) });
      expect(page2Res.status).toBe(200);
      const page2Data = await page2Res.json();
      expect(page2Data.data.length).toBe(1);

      // Test Search Filter
      const searchRes = await getBlogs(makeReq(`http://localhost/api/blogs?search=Article`), { params: Promise.resolve({}) });
      ...
      // Test Status Filter
      const statusRes = await getBlogs(makeReq(`http://localhost/api/blogs?status=draft`), { params: Promise.resolve({}) });
      ...
      const catRes = await getBlogs(makeReq(`http://localhost/api/blogs?category=${techCatIdStr}`), { params: Promise.resolve({}) });
      expect(catRes.status).toBe(200);
      const catData = await catRes.json();
      expect(catData.data.length).toBe(2);

      // Test Uncategorized Default Filter
      const uncategorizedRes = await getBlogs(makeReq(`http://localhost/api/blogs?category=default-uncategorized`), { params: Promise.resolve({}) });
  ...
  describe('Case Studies Endpoint Group', () => {
    it('GET, POST, PATCH, and DELETE work correctly', async () => {
      ...
      const postRes = await postCaseStudy(
        makeReq('http://localhost/api/case-studies', 'POST', mockCaseStudyData(testPortfolioIdStr)),
        { params: Promise.resolve({}) }
      );
      expect(postRes.status).toBe(201);
      const postData = await postRes.json();
      const caseStudyId = postData.id || postData._id;

      const getRes = await getCaseStudies(makeReq('http://localhost/api/case-studies'), { params: Promise.resolve({}) });
      expect(getRes.status).toBe(200);

      const getSingleRes = await getCaseStudy(makeReq(`http://localhost/api/case-studies/${caseStudyId}`), {
        params: Promise.resolve({ id: caseStudyId }),
      });
      expect(getSingleRes.status).toBe(200);

      const patchRes = await patchCaseStudy(
        makeReq(`http://localhost/api/case-studies/${caseStudyId}`, 'PATCH', { projectTitle: 'New Title' }),
        { params: Promise.resolve({ id: caseStudyId }) }
      );
      expect(patchRes.status).toBe(200);

      const deleteRes = await deleteCaseStudy(makeReq(`http://localhost/api/case-studies/${caseStudyId}`, 'DELETE'), {
        params: Promise.resolve({ id: caseStudyId }),
      });
      expect(deleteRes.status).toBe(200);
    });
  ...
  describe('Projects Endpoint Group', () => {
    it('POST, PATCH, and DELETE work correctly', async () => {
      ...
      const postRes = await postProject(
        makeReq('http://localhost/api/projects', 'POST', mockProjectData(testPortfolioIdStr)),
        { params: Promise.resolve({}) }
      );
      expect(postRes.status).toBe(201);
      const postData = await postRes.json();
      const projectId = postData.id || postData._id;

      const getRes = await getProjects(makeReq('http://localhost/api/projects'), { params: Promise.resolve({}) });
      expect(getRes.status).toBe(200);

      const getSingleRes = await getProject(makeReq(`http://localhost/api/projects/${projectId}`), {
        params: Promise.resolve({ id: projectId }),
      });
      expect(getSingleRes.status).toBe(200);

      const patchRes = await patchProject(
        makeReq(`http://localhost/api/projects/${projectId}`, 'PATCH', { title: 'Updated Project' }),
        { params: Promise.resolve({ id: projectId }) }
      );
      expect(patchRes.status).toBe(200);

      const deleteRes = await deleteProject(makeReq(`http://localhost/api/projects/${projectId}`, 'DELETE'), {
        params: Promise.resolve({ id: projectId }),
      });
      expect(deleteRes.status).toBe(200);
    });
  ...
  describe('FAQs Endpoint Group', () => {
    it('POST, GET, PATCH, and DELETE work correctly', async () => {
      ...
      const postRes = await postFaq(
        makeReq('http://localhost/api/faqs', 'POST', mockFaqData(testPortfolioIdStr)),
        { params: Promise.resolve({}) }
      );
      expect(postRes.status).toBe(201);
      const postData = await postRes.json();
      const faqId = postData.id || postData._id;

      const getRes = await getFaqs(makeReq('http://localhost/api/faqs'), { params: Promise.resolve({}) });
      expect(getRes.status).toBe(200);

      const getSingleRes = await getFaq(makeReq(`http://localhost/api/faqs/${faqId}`), {
        params: Promise.resolve({ id: faqId }),
      });
      expect(getSingleRes.status).toBe(200);

      const patchRes = await patchFaq(
        makeReq(`http://localhost/api/faqs/${faqId}`, 'PATCH', { question: 'New FAQ Question' }),
        { params: Promise.resolve({ id: faqId }) }
      );
      expect(patchRes.status).toBe(200);

      const deleteRes = await deleteFaq(makeReq(`http://localhost/api/faqs/${faqId}`, 'DELETE'), {
        params: Promise.resolve({ id: faqId }),
      });
      expect(deleteRes.status).toBe(200);
    });
  ...
  describe('Testimonials Endpoint Group', () => {
    it('POST, GET, PATCH, and DELETE work correctly', async () => {
      ...
      const postRes = await postTestimonial(
        makeReq('http://localhost/api/testimonials', 'POST', mockTestimonialData(testPortfolioIdStr)),
        { params: Promise.resolve({}) }
      );
      expect(postRes.status).toBe(201);
      const postData = await postRes.json();
      const testId = postData.id || postData._id;

      const getRes = await getTestimonials(makeReq('http://localhost/api/testimonials'), { params: Promise.resolve({}) });
      expect(getRes.status).toBe(200);

      const getSingleRes = await getTestimonial(makeReq(`http://localhost/api/testimonials/${testId}`), {
        params: Promise.resolve({ id: testId }),
      });
      expect(getSingleRes.status).toBe(200);

      const patchRes = await patchTestimonial(
        makeReq(`http://localhost/api/testimonials/${testId}`, 'PATCH', { name: 'John Doe Updated' }),
        { params: Promise.resolve({ id: testId }) }
      );
      expect(patchRes.status).toBe(200);

      const deleteRes = await deleteTestimonial(makeReq(`http://localhost/api/testimonials/${testId}`, 'DELETE'), {
        params: Promise.resolve({ id: testId }),
      });
      expect(deleteRes.status).toBe(200);
    });
  ...
  describe('Categories Endpoint Group', () => {
    it('POST, GET, PATCH, and DELETE work correctly', async () => {
      ...
      const postRes = await postCategory(
        makeReq('http://localhost/api/categories', 'POST', mockCategoryData(testPortfolioIdStr)),
        { params: Promise.resolve({}) }
      );
      expect(postRes.status).toBe(201);
      const postData = await postRes.json();
      const catId = postData.id || postData._id;

      const getRes = await getCategories(makeReq('http://localhost/api/categories'), { params: Promise.resolve({}) });
      expect(getRes.status).toBe(200);
      const data = await getRes.json();
      expect(data.data.length).toBe(2); // uncategorized default + our Tech category

      const patchRes = await patchCategory(
        makeReq(`http://localhost/api/categories/${catId}`, 'PATCH', { name: 'Technology' }),
        { params: Promise.resolve({ id: catId }) }
      );
      expect(patchRes.status).toBe(200);

      const deleteRes = await deleteCategory(makeReq(`http://localhost/api/categories/${catId}`, 'DELETE'), {
        params: Promise.resolve({ id: catId }),
      });
      expect(deleteRes.status).toBe(200);
    });
  ...
  describe('Portfolios Endpoint Group', () => {
    it('POST, GET, GET single, Config, and mutations work correctly', async () => {
      ...
      // GET List
      const getRes = await getPortfolios(makeReq('http://localhost/api/portfolios'), { params: Promise.resolve({}) });
      expect(getRes.status).toBe(200);
      const data = await getRes.json();
      expect(data.length).toBe(2);

      // GET Single
      const getSingleRes = await getPortfolio(makeReq(`http://localhost/api/portfolios/${testPortfolioIdStr}`), {
      ...
      const patchRes = await patchPortfolio(
        makeReq(`http://localhost/api/portfolios/${testPortfolioIdStr}`, 'PATCH', { name: 'Renamed Portfolio' }),
        { params: Promise.resolve({ id: testPortfolioIdStr }) }
      );
      expect(patchRes.status).toBe(200);
      ...
      // DELETE
      const deleteRes = await deletePortfolio(makeReq(`http://localhost/api/portfolios/${testPortfolioIdStr}`, 'DELETE'), {
        params: Promise.resolve({ id: testPortfolioIdStr }),
      });
      expect(deleteRes.status).toBe(200);
    });
  ...
  describe('Authentication Endpoint Group', () => {
    it('POST /api/auth/login, me, logout, password flow works', async () => {
      ...
      // Me
      mockCookieToken = testAdminToken;
      const meRes = await authMe(makeReq('http://localhost/api/auth/me'), { params: Promise.resolve({}) });
      ...
      const user = await mongoose.connection.db.collection('users').findOne({ email: mockAdminUser.email });
      ...
      // Logout
      const logoutRes = await logout(makeReq('http://localhost/api/auth/logout', 'POST'), { params: Promise.resolve({}) });
  ...
  describe('Leads Endpoint Group', () => {
    it('POST /api/leads/capture, POST /api/leads, export, and mutations work', async () => {
      ...
      const csResult = await mongoose.connection.db.collection(CollectionName.CASE_STUDIES).insertOne({
      ...
      const captureRes = await captureLead(
        makeReq('http://localhost/api/leads/capture', 'POST', mockLeadCaptureData(testPortfolioIdStr, testCaseStudyIdStr)),
        { params: Promise.resolve({}) }
      );
      ...
      const postRes = await postLead(
        makeReq('http://localhost/api/leads', 'POST', mockLeadData(testPortfolioIdStr)),
        { params: Promise.resolve({}) }
      );
      expect(postRes.status).toBe(201);
      const postLeadData = await postRes.json();
      const leadId = postLeadData.id || postLeadData._id;

      // GET leads
      const getRes = await getLeads(makeReq('http://localhost/api/leads'), { params: Promise.resolve({}) });
      expect(getRes.status).toBe(200);

      // GET single lead
      const getSingleRes = await getLead(makeReq(`http://localhost/api/leads/${leadId}`), {
      ...
      const patchRes = await patchLead(
        makeReq(`http://localhost/api/leads/${leadId}`, 'PATCH', { firstName: 'Robert' }),
        { params: Promise.resolve({ id: leadId }) }
      );
      expect(patchRes.status).toBe(200);

      // Export Leads
      const exportRes = await exportLeads(makeReq('http://localhost/api/leads/export'), { params: Promise.resolve({}) });
      expect(exportRes.status).toBe(200);

      // DELETE lead
      const deleteRes = await deleteLead(makeReq(`http://localhost/api/leads/${leadId}`, 'DELETE'), {
        params: Promise.resolve({ id: leadId }),
      });
      expect(deleteRes.status).toBe(200);
    });
  ...
  describe('Subscribers Endpoint Group', () => {
    it('mutations, unsubscribe, GET /api/subscribers work correctly', async () => {
      ...
      const subResult = await mongoose.connection.db.collection(CollectionName.SUBSCRIBERS).insertOne({
      ...
      // GET list
      mockCookieToken = testAdminToken;
      const getRes = await getSubscribers(makeReq('http://localhost/api/subscribers'), { params: Promise.resolve({}) });
      expect(getRes.status).toBe(200);

      // PATCH
      const patchRes = await patchSubscriber(
        makeReq(`http://localhost/api/subscribers/${subId}`, 'PATCH', { status: 'unsubscribed' }),
      ...
      // DELETE
      const deleteRes = await deleteSubscriber(makeReq(`http://localhost/api/subscribers/${subId}`, 'DELETE'), {
        params: Promise.resolve({ id: subId }),
      });
      expect(deleteRes.status).toBe(200);
    });
  ...
  describe('Campaigns Endpoint Group', () => {
    it('POST, GET, send, and DELETE campaigns work correctly', async () => {
      ...
      await mongoose.connection.db.collection(CollectionName.SUBSCRIBERS).insertOne({
      ...
      const postRes = await postCampaign(
        makeReq('http://localhost/api/campaigns', 'POST', mockCampaignData(testPortfolioIdStr)),
        { params: Promise.resolve({}) }
      );
      expect(postRes.status).toBe(201);
      const postData = await postRes.json();
      const campaignId = postData.id || postData._id;

      const getRes = await getCampaigns(makeReq('http://localhost/api/campaigns'), { params: Promise.resolve({}) });
      expect(getRes.status).toBe(200);

      const getSingleRes = await getCampaign(makeReq(`http://localhost/api/campaigns/${campaignId}`), {
        params: Promise.resolve({ id: campaignId }),
      });
      expect(getSingleRes.status).toBe(200);

      // Trigger Send Campaign
      const sendRes = await sendCampaign(makeReq(`http://localhost/api/campaigns/${campaignId}/send`, 'POST'), {
        params: Promise.resolve({ id: campaignId }),
      });
      expect(sendRes.status).toBe(200);

      const deleteRes = await deleteCampaign(makeReq(`http://localhost/api/campaigns/${campaignId}`, 'DELETE'), {
        params: Promise.resolve({ id: campaignId }),
      });
      expect(deleteRes.status).toBe(200);
    });
  ...
  describe('Media Endpoint Group', () => {
    it('GET /api/media, POST metadata, PATCH single, bulk DELETE, upload, and delete work correctly', async () => {
      ...
      // Media Auth
      const authRes = await getMediaAuth(makeReq('http://localhost/api/media/auth'), { params: Promise.resolve({}) });
      ...
      const uploadRes = await uploadMediaProxy(uploadReq, { params: Promise.resolve({}) });
      expect(uploadRes.status).toBe(201);
      const uploadedMediaId = (await uploadRes.json())._id;

      // POST /api/media (creates metadata)
      const postRes = await postMedia(
        makeReq('http://localhost/api/media', 'POST', mockMediaData(testPortfolioIdStr)),
        { params: Promise.resolve({}) }
      );
      expect(postRes.status).toBe(201);
      const postMediaData = await postRes.json();
      const postedMediaId = postMediaData.id || postMediaData._id;

      // PATCH /api/media/[id]
      const patchRes = await patchMedia(
        makeReq(`http://localhost/api/media/${postedMediaId}`, 'PATCH', { altText: 'Stunning Image' }),
        { params: Promise.resolve({ id: postedMediaId }) }
      );
      expect(patchRes.status).toBe(200);

      // GET Media
      const getRes = await getMedia(makeReq('http://localhost/api/media'), { params: Promise.resolve({}) });
      expect(getRes.status).toBe(200);

      // Bulk DELETE /api/media
      const bulkDeleteRes = await bulkDeleteMedia(
        makeReq('http://localhost/api/media', 'DELETE', { ids: [postedMediaId] }),
        { params: Promise.resolve({}) }
      );
      expect(bulkDeleteRes.status).toBe(200);

      // DELETE Single Media (uploaded one)
      const deleteRes = await deleteMedia(makeReq(`http://localhost/api/media/${uploadedMediaId}`, 'DELETE'), {
  ...
  describe('Analytics Endpoint Group', () => {
    it('POST /api/analytics/collect works', async () => {
      const res = await collectAnalytics(
        makeReq('http://localhost/api/analytics/collect', 'POST', mockAnalyticsData(testPortfolioIdStr)),
        { params: Promise.resolve({}) }
      );
  ...
  describe('Public Portfolio Lead and Newsletter subscriptions', () => {
    it('captures leads and newsletter subscriptions for a portfolio', async () => {
      ...
      const leadRes = await submitPortfolioLead(
        makeReq(`http://localhost/api/portfolios/${testPortfolioIdStr}/leads`, 'POST', mockLeadCaptureData(testPortfolioIdStr, testCaseStudyIdStr)),
        { params: Promise.resolve({ id: testPortfolioIdStr }) }
      );
      ...
      const subRes = await subscribePortfolio(
        makeReq(`http://localhost/api/portfolios/${testPortfolioIdStr}/subscribe`, 'POST', {
